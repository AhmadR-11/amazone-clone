...
import { Eye, EyeOff, ArrowRight, KeyRound, CheckCircle, ShieldCheck } from 'lucide-react';
  ...
  const handleRequestSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setInfo(`Verification code dispatched to ${email.trim()}. Please check your email.`);
      }
    } catch {
      ...
      {step === 'request' && (
        <>
          ...
          <form onSubmit={handleRequestSubmit} className="space-y-4">
            ...
            >
              <span>{loading ? 'Verifying Email...' : 'SEND VERIFICATION CODE'}</span>
        ...
        <>
          ...
          <form onSubmit={handleResetSubmit} className="space-y-4">
            ...
            <button
              ...
              className="w-full bg-slate-950 hover:bg-blue-600 text-white font-black text-xs uppercase tracking-wider py-3.5 rounded-full transition-all duration-300 shadow-md hover:shadow-blue-500/25 hover:scale-[1.01] active:scale-[0.99] cursor-pointer flex items-center justify-center gap-2 disabled:opacity-50 mt-2"
            >
      ...
      {step === 'success' && (
        <div className="text-center py-4 space-y-4">
          ...
          <div className="pt-2">
            <Link
              href={`/auth/login?redirect=${encodeURIComponent(redirectUrl)}`}
              className="inline-flex items-center justify-center gap-2 w-full bg-slate-950 hover:bg-blue-600 text-white font-black text-xs uppercase tracking-wider py-3.5 rounded-full transition-all duration-300 shadow-md hover:shadow-blue-500/25 hover:scale-[1.01] active:scale-[0.99] cursor-pointer"
            >
    ...
    <div className="min-h-[100dvh] bg-[#050b18] flex items-center justify-center p-4 sm:p-6 lg:p-8 relative overflow-hidden">
      ...
      <div className="w-full max-w-5xl bg-white/90 backdrop-blur-2xl rounded-[32px] shadow-[0_30px_70px_-15px_rgba(0,0,0,0.6)] border border-white/50 overflow-hidden flex flex-col lg:flex-row relative z-10 my-6">
        ...
        <div className="lg:w-[45%] bg-slate-950/95 p-8 sm:p-10 lg:p-12 relative flex flex-col justify-between overflow-hidden text-white min-h-[380px] lg:min-h-[540px] border-r border-white/10">
          ...
          <div className="relative z-10 my-auto py-8">
            ...
            <p className="text-xs sm:text-sm text-slate-300 font-medium leading-relaxed max-w-md">
              Verify your email with a 6-digit one-time passkey to securely update your password.
            </p>
          </div>
        ...
        <div className="lg:w-[55%] p-8 sm:p-10 lg:p-12 flex flex-col justify-center bg-white/95 backdrop-blur-xl">
          <Suspense fallback={<div className="text-center text-slate-500 text-xs py-8">Loading recovery console...</div>}>
            <ForgotPasswordForm />
          </Suspense>
        </div>
      </div>
